...
y <- DGEList(counts=x,group=factor(samplenames))
pdf("Hw_edgeR.pdf")
dim(y)
y.full <- y
...
toptags <- topTags(et12,n=500,adjust.method="BH", sort.by="PValue")
colnames(toptags)
write.csv(toptags,file="toptags_0vs10.csv")

toptags <- topTags(et13,n=500, adjust.method="BH", sort.by="PValue")
write.csv(toptags,file="toptags_0vs20.csv")

toptags <- topTags(et23,n=200,adjust.method="BH", sort.by="PValue")
